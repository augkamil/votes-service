class Vote < ActiveRecord::Base
  attr_accessible :entry_id, :user_id, :value
  ...
  named_scope :entry_id, lambda {|entry_id| {:conditions => ["entry_id = ?",entry_id]}}
  
  def self.create_or_update(attributes)
    vote = Vote.find_by_entry_id_and_user_id(attributes[:entry_id],attributes[:user_id])
    if vote
      vote.value = attribute[:value]
      vote.save
      vote
    else
  ...
  def self.entry_ids_for_user(user_id, value, page, per_page)
    votes = paginate_by_user_id_and_value(user_id, value, :page => page, :per_page => per_page)
