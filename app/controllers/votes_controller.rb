class VotesController < ApplicationController
  ...
  def create 
    begin
      ...
      vote = Vote.create_or_update(:user_id => params["user_id"], :entry_id => params["entry_id"], :value => json["value"])
     ...
     def entry_ids_voted_up_for_user
       ...
       data[:previous_page] = user_up_votes_url(:user_id => user_id, :page => page -1, :per_page => per_page) if page > 1
       data[:next_page] = user_up_votes_url(:user_id => user_id, :page => page + 1, :per_page => per_page ) if (page*per_page) < count
       ...
       def entry_ids_voted_down_for_user
         ...
         data[:previous_page] = user_down_votes_url(:user_id => user_id, :page => page -1, :per_page => per_page) if page > 1
         data[:next_page] = user_down_votes_url(:user_id => user_id, :page => page + 1, :per_page => per_page) if (page*per_page) < count
         
         render :json => data.to_json 
         
       end
       
       def total_for_entries
